...
class MissionComputer:
    ...
    def __init__(self):
        # 1. 화성 기지의 환경에 대한 값을 저장할 수 있는 사전 객체 포함
        self.env_values = {
            'mars_base_internal_temperature': 0.0,
            'mars_base_external_temperature': 0.0,
            'mars_base_internal_humidity': 0.0,
        ...
        # 2. 문제 3에서 제작한 DummySensor 클래스를 ds라는 이름으로 인스턴스화
        self.ds = DummySensor()
    ...
    def get_sensor_data(self):
        '''
        미션 컴퓨터의 센서 데이터를 지속적으로 수집 및 출력하는 메소드
        '''
        # 보너스 과제: 특정 키(q) 입력 시 중지
...
if __name__ == '__main__':
    # 4. MissionComputer 클래스를 RunComputer 라는 이름으로 인스턴스화
    RunComputer = MissionComputer()

    # 5. RunComputer 인스턴스의 get_sensor_data() 메소드 호출
